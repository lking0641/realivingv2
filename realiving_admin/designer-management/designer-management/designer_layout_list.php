...
function renderCardMeta($c) { ?>
    <div class="flex flex-wrap gap-1.5 mt-2.5 items-center">
        ...
        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-line bg-[#F5F5F5] text-ink"><i class="fas fa-user-tie text-[8px]"></i> Lead: <?= htmlspecialchars($c['designer1_name']) ?></span>
        <?php endif; ?>
        <?php if ($c['designer2_name']): ?>
        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-blue-200 bg-blue-50 text-blue-700"><i class="fas fa-user text-[8px]"></i> Support: <?= htmlspecialchars($c['designer2_name']) ?></span>
        <?php endif; ?>
        <?php if (!empty($c['intake_id'])): ?>
            <?php if ($c['layout_type_2d']): ?><span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-purple-300 bg-purple-100 text-purple-800"><i class="fas fa-vector-square text-[8px]"></i> 2D</span><?php endif; ?>
            <?php if ($c['layout_type_3d']): ?><span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-purple-300 bg-purple-100 text-purple-800"><i class="fas fa-cube text-[8px]"></i> 3D</span><?php endif; ?>
            <span class="text-[10px] text-muted"><i class="fas fa-clock text-[9px]"></i> Submitted <?= date('M d, Y', strtotime($c['intake_submitted_at'])) ?></span>
        <?php endif; ?>
        ...
        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-red-300 bg-red-100 text-red-800 inline-flex items-center gap-1">
            <i class="fas fa-times-circle text-[9px]"></i> <?= $c['rejected_layouts'] ?> Rejected
        </span>
        <?php endif; ?>
    </div>
<?php }
    ...
    <div class="max-w-[1100px] mx-auto px-5 py-8">
        ...
        <div class="bg-white border border-line rounded-[10px] p-6 mb-5">
            <div class="text-[11px] font-semibold tracking-[1.5px] uppercase text-soft mb-2">
                <i class="fas fa-drafting-compass"></i> 2D / 3D Layout
            </div>
            ...
            <?php else: ?>
                <div class="bg-white border border-line rounded-[10px] p-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-[10px] font-semibold uppercase tracking-[0.4px] text-muted">Intake Done</div>
                    </div>
                </div>
            <?php endif; ?>
            <div class="bg-white border border-line rounded-[10px] p-4 flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-[10px] font-semibold uppercase tracking-[0.4px] text-muted">Intake Pending</div>
                </div>
            </div>
            <div class="bg-white border border-line rounded-[10px] p-4 flex items-center gap-3">
        ...
        <?php if ($isHead && !empty($myDesigners)): ?>
            <div class="bg-white border border-line rounded-[10px] p-6 mb-5">
                <div class="flex items-center gap-2.5 text-xs font-semibold mb-4">
                    <i class="fas fa-users-cog text-soft"></i> My Design Team
                    ...
                    ?>
                        <div class="bg-[#F5F5F5] border border-line rounded-lg px-3.5 py-2.5 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-[10px] text-muted capitalize"><?= str_replace('_', ' ', $d['role']) ?></div>
        ...
        <div class="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <?php endif; ?>
            <button class="filter-tab inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-[13px] font-semibold transition border bg-white text-soft border-line"
                data-filter="intake-pending" onclick="setFilter('intake-pending', this)">
                <i class="fas fa-hourglass-half"></i> Intake Pending
                <span class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-[#F5F5F5] text-ink"><?= $intakePending ?></span>
            </button>
            <button class="filter-tab inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-[13px] font-semibold transition border bg-white text-soft border-line"
        ...
        <?php if (empty($clients)): ?>
            <div class="bg-white border border-line rounded-[10px] p-6">
                <div class="text-center py-10 text-muted">
                    ...
                    <p class="text-[12.5px] mt-1">You have no clients assigned for 2D/3D layout work yet.</p>
                </div>
            </div>
        <?php else: ?>
            ...
            <div id="section-active" class="flex flex-col gap-3">
                <?php if ($canViewAll && !empty($myClients)): ?>
                    ...
                    <?php foreach ($myClients as $c):
                        ...
                        $rejected  = !empty($c['rejected_layouts']) && $c['rejected_layouts'] > 0;
                    ?>
                        <div class="client-card bg-white border border-line <?= $rejected ? 'border-red-400' : '' ?> border-l-4 border-l-amber-400 rounded-lg cursor-pointer hover:border-ink transition"
                            ...
                            <div class="flex items-stretch">
                                <div class="flex-1 min-w-0 p-4">
                                    <div class="flex justify-between items-start gap-2.5 mb-1 flex-wrap">
                                        ...
                                        <div>
                                            <?php if ($hasIntake): ?>
                                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border bg-emerald-100 text-emerald-800 border-emerald-300"><i class="fas fa-check-circle"></i> Intake Done</span>
                                            <?php else: ?>
                                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border bg-amber-100 text-amber-800 border-amber-300"><i class="fas fa-hourglass-half"></i> Intake Pending</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                ...
                                <div class="hidden sm:flex items-center px-4 border-l border-line flex-shrink-0">
                                    <a href="<?= BASE_URL ?>designer-2d3d-layout?client_id=<?= $c['id'] ?>"
                ...
                ?>
                    <div class="client-card bg-white border border-line <?= $rejected ? 'border-red-400' : '' ?> <?= $stripe ?> border-l-4 rounded-lg cursor-pointer hover:border-ink transition"
                ...
                <?php if (empty($activeClients)): ?>
                    <div class="bg-white border border-line rounded-[10px] p-6">
                        <div class="text-center py-10 text-muted">
                            ...
                            <p class="text-[12.5px] mt-1">No active clients — all layouts are done. Check the Done tab.</p>
                        </div>
                    </div>
                <?php endif; ?>
            </div><!-- /section-active -->
            ...
            <div id="section-done" class="hidden flex-col gap-3">
                <?php if (!empty($doneClients)): ?>
                    <div class="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.6px] text-muted mt-1">
                        <i class="fas fa-flag-checkered"></i> Completed Layouts <span class="text-[11px]">(<?= count($doneClients) ?>)</span>
                        <span class="flex-1 h-px bg-line"></span>
                    </div>
                            ...
                            <div class="flex items-stretch">
                                <div class="flex-1 min-w-0 p-4">
                                    <div class="flex justify-between items-start gap-2.5 mb-1 flex-wrap">
                                        ...
                                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border bg-teal-100 text-teal-800 border-teal-300"><i class="fas fa-flag-checkered"></i> Layout Done</span>
                                    </div>
                                    <?php renderCardMeta($c); ?>
                                </div>
                                <div class="hidden sm:flex items-center px-4 border-l border-line flex-shrink-0">
                ...
                <?php else: ?>
                    <div class="bg-white border border-line rounded-[10px] p-6">
                        <div class="text-center py-10 text-muted">
                            ...
                            <p class="text-[15px] font-semibold text-ink">No Completed Layouts Yet</p>
                            <p class="text-[12.5px] mt-1">Clients whose 2D/3D Layout stage is marked Done will appear here.</p>
                        </div>
                    </div>
                <?php endif; ?>
            </div><!-- /section-done -->
