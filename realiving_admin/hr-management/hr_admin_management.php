...
$where = ["role != 'super_admin'"]; // HR never sees super_admin accounts
$params = [];
$types = '';
...
// Distinct roles for filter dropdown (super_admin excluded)
$roles = [];
...
<head>
  ...
  <title>Employee Management - RealLiving</title>
  ...
  <div class="pt-10 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
    ...
    <div class="mb-8 adm-fade flex items-start justify-between flex-wrap gap-4">
      <div>
        ...
        <h1 class="adm-title">Employee Management</h1>
        <p class="adm-subtitle mt-1">View, edit, and manage access for employee accounts.</p>
      </div>
      <a href="<?= BASE_URL ?>human-resource-dashboard" class="adm-btn"><i class="fas fa-arrow-left"></i> Back to Dashboard</a>
    </div>
    ...
    <form method="GET" class="adm-filterbar mb-6 adm-fade flex flex-wrap gap-3 items-center">
      ...
      <?php if ($search || $role_filter || $status_filter): ?>
        <a href="<?= BASE_URL ?>hr-admin-management" class="adm-btn">Clear</a>
      <?php endif; ?>
    </form>
    ...
    <div class="adm-table-wrap adm-fade">
      <?php if (empty($admins)): ?>
        <div class="adm-empty">
          ...
          <p>No employee accounts match your filters.</p>
        </div>
      <?php else: ?>
  ...
  <script>
    ...
    document.querySelectorAll('.toggle-status-btn').forEach(btn => {
      btn.addEventListener('click', function () {
        ...
        showConfirm(
          `${actionWord.charAt(0).toUpperCase() + actionWord.slice(1)} employee?`,
