  ...
  <div>
    ...
    <input type="number" id="labor_cost" name="labor_cost" step="0.01"
      ...
      value="<?= isset($item['labor_cost']) ? floatval($item['labor_cost']) : '' ?>">
              ...
              <!-- Room/Unit Distribution (All business types) -->
              <div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg" style="display:none;">
                ...
                <!-- Distribution Summary (hidden by default) -->
                <div id="distribution-summary" class="mt-3 hidden">
                  <div class="bg-white p-3 rounded border border-blue-300">
                    ...
                    <div id="distribution-summary-list" class="text-sm text-gray-600"></div>
                  </div>
                </div>
              </div>
...
// Room/Unit Distribution Functionality
let roomDistributionData = [];

function openRoomDistributionModal() {
  const totalQty = parseInt(document.getElementById('total_quantity').value) || 1;
  ...
  // Initialize with existing distribution or create new with default unit 1
  ...
  const modal = document.getElementById('roomDistributionModal');
  if (modal) {
...
function generateRoomInputs() {
  ...
  const container = document.getElementById('room-inputs-container');
  
  if (!container) return;
  
  // Preserve existing data or create new entries
  const newData = [];
  for (let i = 0; i < numRooms; i++) {
    if (roomDistributionData[i]) {
      newData.push(roomDistributionData[i]);
    } else {
      // Default to next available unit number
      const nextUnitNumber = i + 1;
      newData.push({
        room_number: nextUnitNumber,
        room_name: '',
        quantity: 0,
        notes: ''
      });
      ...
      <div class="border border-gray-300 rounded-lg p-4 bg-white">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="number" 
                   class="room-number-input w-full px-3 py-2 border rounded-md text-sm font-semibold"
                   data-index="${index}"
                   value="${room.room_number}"
                   min="1"
                   placeholder="e.g., 1, 2, 4, 101"
                   onchange="updateRoomData(${index}, 'room_number', parseInt(this.value) || 1)"
                   style="border-color: #6366f1; background: #f0f9ff;">
            <p class="text-xs text-gray-500 mt-1">
              <i class="fas fa-info-circle mr-1"></i>
              You can set any unit number
            </p>
          </div>
          ...
          <div>
            ...
            <input type="text" 
                   class="room-name-input w-full px-3 py-2 border rounded-md text-sm"
                   data-index="${index}"
                   value="${room.room_name || ''}"
                   placeholder="e.g., Master Bedroom, Unit 101"
        ...
        <div class="mt-3">
          ...
          <input type="number" 
                 class="room-qty-input w-full px-3 py-2 border rounded-md text-sm"
                 data-index="${index}"
                 value="${room.quantity}"
                 min="0"
        ...
        <div class="mt-2">
          ...
          <textarea class="room-notes-input w-full px-3 py-2 border rounded-md text-sm"
                    data-index="${index}"
                    rows="2"
                    placeholder="Additional notes for this room/unit"
...
function updateRoomData(index, field, value) {
  if (roomDistributionData[index]) {
    ...
    // If updating room_number, ensure it's valid
...
function validateDistribution() {
  const totalQty = parseInt(document.getElementById('total_quantity').value) || 1;
  const distributedTotal = roomDistributionData.reduce((sum, room) => sum + (room.quantity || 0), 0);
  ...
  if (totalDisplay) {
    totalDisplay.textContent = distributedTotal;
  }
  ...
  if (duplicates.length > 0) {
    ...
    errorText.textContent = `Duplicate unit number found: ${duplicates[0]}. Each unit must have a unique number.`;
  ...
  if (distributedTotal > totalQty) {
    errorDiv.classList.remove('hidden');
    errorText.textContent = `Distributed quantity (${distributedTotal}) exceeds total quantity (${totalQty})`;
    totalDisplay.classList.add('text-red-600');
    totalDisplay.classList.remove('text-green-600', 'text-orange-600');
    return false;
  } else if (distributedTotal < totalQty) {
    errorDiv.classList.remove('hidden');
    errorText.textContent = `${totalQty - distributedTotal} items not yet distributed`;
    totalDisplay.classList.add('text-orange-600');
    totalDisplay.classList.remove('text-green-600', 'text-red-600');
    return true; // Allow partial distribution
  } else {
...
function distributeEvenly() {
  ...
  roomDistributionData.forEach((room, index) => {
    room.quantity = baseQty + (index < remainder ? 1 : 0);
  });
...
function setOnePerRoom() {
  ...
  if (numRooms > totalQty) {
    alert(`Cannot set 1 per room. You only have ${totalQty} items but ${numRooms} rooms.`);
    return;
  }
  
  roomDistributionData.forEach((room, index) => {
    room.quantity = index < totalQty ? 1 : 0;
  });
...
function setTwoPerRoom() {
  const totalQty = parseInt(document.getElementById('total_quantity').value) || 1;
  const numRooms = roomDistributionData.length;
  const needed = numRooms * 2;
  
  if (needed > totalQty) {
    alert(`Cannot set 2 per room. You need ${needed} items but only have ${totalQty}.`);
    return;
  }
...
function applyRoomDistribution() {
  if (!validateDistribution()) {
    ...
    if (distributedTotal > totalQty) {
      alert('Cannot apply: Distributed quantity exceeds total quantity.');
      return;
    }
    ...
    const duplicates = unitNumbers.filter((num, idx) => unitNumbers.indexOf(num) !== idx);
    if (duplicates.length > 0) {
      alert(`Cannot apply: Duplicate unit numbers found. Please ensure each unit has a unique number.`);
      return;
    }
  }
  
  // Filter out rooms with 0 quantity
  const activeRooms = roomDistributionData.filter(room => room.quantity > 0);
  
  if (activeRooms.length === 0) {
    alert('Please assign quantities to at least one room/unit.');
    return;
  }
  
  // Store distribution data as hidden fields
  const form = document.querySelector('form');
  
  // Remove old distribution fields
  form.querySelectorAll('[name^="room_distribution"]').forEach(el => el.remove());
  
  // Add new distribution fields
  activeRooms.forEach((room, index) => {
    const fields = ['room_number', 'room_name', 'quantity', 'notes'];
...
function updateDistributionSummary(rooms) {
  ...
  rooms.forEach(room => {
    const roomLabel = room.room_name || `Unit ${room.room_number}`;
    html += `
      <div class="flex justify-between text-xs">
        <span><i class="fas fa-door-open mr-1 text-blue-500"></i>${roomLabel} (Unit #${room.room_number})</span>
...
function updateTotalQuantityFromModal(newVal) {
  ...
  // Update the main quantity field on the form
  const mainQtyInput = document.getElementById('total_quantity');
  if (mainQtyInput) {
    mainQtyInput.value = qty;
  }
  
  // Sync all selected addon quantities
  syncAddonQuantities(qty);
...
document.getElementById('total_quantity')?.addEventListener('change', function() {
  ...
  if (maxQtyDisplay) {
    maxQtyDisplay.textContent = newQty;
  }
  
  // Also keep modal total input in sync if modal has been opened
  const modalInput = document.getElementById('distribution-total-input');
  if (modalInput) {
    modalInput.value = newQty;
    document.getElementById('distribution-max').textContent = newQty;
  }

  // Sync addon quantities
  syncAddonQuantities(newQty);
  
  // If distribution is active, validate it
  if (roomDistributionData.length > 0) {
    validateDistribution();
  }
});
...
<div id="roomDistributionModal" class="modal" style="display: none;">
  <div class="modal-content" style="max-width: 700px;">
    <div class="modal-header">
      <h2><i class="fas fa-building mr-2"></i>Room/Unit Distribution</h2>
      <button onclick="closeRoomDistributionModal()" class="modal-close">
    ...
    <div class="p-6">
      ...
      <div class="mt-4 p-3 bg-indigo-600 rounded-lg">
        <div class="flex justify-between items-center gap-3">
          ...
          <input type="number" 
                 id="distribution-total-input"
                 min="1"
                 class="w-24 text-center text-lg font-bold text-indigo-700 border-2 border-indigo-300 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                 value="1"
                 onchange="updateTotalQuantityFromModal(this.value)" />
        </div>
        <p class="text-xs text-indigo-200 mt-1">
          <i class="fas fa-info-circle mr-1"></i>
          Editing this will also update the main quantity field
        </p>
        <div class="mt-2">
