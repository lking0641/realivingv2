...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_edit'])) {
    if ($quotationDone) {
        ...
        if (!$is_fixed) {
            ...
            // Handle room distribution
            $delRooms = $conn->prepare("DELETE FROM quotation_room_distribution WHERE quotation_entry_id = ?");
            $delRooms->bind_param("i", $entry_id);
            $delRooms->execute();

            if (!empty($_POST['room_distribution']) && is_array($_POST['room_distribution'])) {
                $insRoom = $conn->prepare("INSERT INTO quotation_room_distribution (quotation_entry_id, room_unit_number, room_unit_name, quantity, notes) VALUES (?, ?, ?, ?, ?)");
                foreach ($_POST['room_distribution'] as $room) {
                    $rNum  = intval($room['room_number'] ?? 0);
                    $rName = trim($room['room_name'] ?? '');
                    $rQty  = intval($room['quantity'] ?? 0);
                    $rNote = trim($room['notes'] ?? '');
                    if ($rQty > 0) {
                        $insRoom->bind_param("iisis", $entry_id, $rNum, $rName, $rQty, $rNote);
                        $insRoom->execute();
                    }
                }
            }
...
foreach ($_POST['addon_selected'] as $addonId => $isSelected) {
    if ($isSelected == '1') {
        ...
        $insAddon->execute();
            ...
            // Handle room distribution for fixed size
            $delRoomsFixed = $conn->prepare("DELETE FROM quotation_room_distribution WHERE quotation_fixed_size_id = ?");
            $delRoomsFixed->bind_param("i", $fixed_id);
            $delRoomsFixed->execute();

            if (!empty($_POST['room_distribution']) && is_array($_POST['room_distribution'])) {
                $insRoomFixed = $conn->prepare("INSERT INTO quotation_room_distribution (quotation_fixed_size_id, room_unit_number, room_unit_name, quantity, notes) VALUES (?, ?, ?, ?, ?)");
                foreach ($_POST['room_distribution'] as $room) {
                    $rNum  = intval($room['room_number'] ?? 0);
                    $rName = trim($room['room_name'] ?? '');
                    $rQty  = intval($room['quantity'] ?? 0);
                    $rNote = trim($room['notes'] ?? '');
                    if ($rQty > 0) {
                        $insRoomFixed->bind_param("iisis", $fixed_id, $rNum, $rName, $rQty, $rNote);
                        $insRoomFixed->execute();
                    }
                }
            }
...
<div class="max-w-5xl mx-auto px-4 py-8">
  ...
  <form method="POST">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
      ...
      <?php
// Fetch existing room distribution for this entry
$existingRoomDist = [];
if (!$is_fixed && $entry_id) {
    $rdStmt = $conn->prepare("SELECT * FROM quotation_room_distribution WHERE quotation_entry_id = ? ORDER BY room_unit_number");
    $rdStmt->bind_param("i", $entry_id);
    $rdStmt->execute();
    $existingRoomDist = $rdStmt->get_result()->fetch_all(MYSQLI_ASSOC);
} elseif ($is_fixed && $fixed_id) {
    $rdStmt = $conn->prepare("SELECT * FROM quotation_room_distribution WHERE quotation_fixed_size_id = ? ORDER BY room_unit_number");
    $rdStmt->bind_param("i", $fixed_id);
    $rdStmt->execute();
    $existingRoomDist = $rdStmt->get_result()->fetch_all(MYSQLI_ASSOC);
}
?>

      <!-- Room/Unit Distribution -->
      <div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg mb-4" style="display:none;">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            <i class="fas fa-building text-blue-600"></i>
            <label class="font-medium text-gray-700">Distribute by Room/Unit (Optional)</label>
          </div>
          <button type="button" onclick="openRoomDistributionModal()"
        ...
        <p class="text-xs text-gray-600">
          ...
          Optionally split the total quantity across multiple rooms or units
        </p>

        <!-- Distribution Summary -->
        <div id="distribution-summary" class="mt-3 <?= empty($existingRoomDist) ? 'hidden' : '' ?>">
          <div class="bg-white p-3 rounded border border-blue-300">
            <div class="flex items-center justify-between mb-2">
              <span class="text-sm font-semibold text-gray-700">Distribution Active:</span>
              <button type="button" onclick="clearRoomDistribution()"
                      class="text-xs text-red-600 hover:text-red-800 font-medium"
                      <?= $quotationDone ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : '' ?>>
                <i class="fas fa-times-circle mr-1"></i>Clear
              </button>
            </div>
            <div id="distribution-summary-list" class="text-sm text-gray-600">
              <?php foreach ($existingRoomDist as $rd): ?>
                <div class="flex justify-between text-xs mb-1">
                  <span><i class="fas fa-door-open mr-1 text-blue-500"></i>
                    <?= htmlspecialchars($rd['room_unit_name'] ?: 'Unit ' . $rd['room_unit_number']) ?>
                    (Unit #<?= $rd['room_unit_number'] ?>)
                  </span>
                  <span class="font-semibold"><?= $rd['quantity'] ?> items</span>
                </div>
              <?php endforeach; ?>
            </div>
          </div>
...
<input type="hidden" name="addon_category[<?= $addon['id'] ?>]"
    ...
    <!-- Room/Unit Distribution Modal -->
    <div id="roomDistributionModal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5); align-items:center; justify-content:center;">
      <div style="background:#fefefe; padding:30px; border-radius:12px; max-width:700px; width:90%; max-height:90vh; overflow-y:auto; margin:20px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; border-bottom:2px solid #e5e7eb; padding-bottom:15px;">
          <h2 style="font-size:20px; font-weight:bold; color:#1f2937;">
            <i class="fas fa-building mr-2"></i>Room/Unit Distribution
          </h2>
          <button onclick="closeRoomDistributionModal()" style="font-size:24px; color:#666; cursor:pointer; background:none; border:none;">
            <i class="fas fa-times"></i>
          </button>
        </div>
        ...
        <div class="p-2">
          <div class="mb-4">
            <label class="block text-sm font-semibold mb-2">Number of Rooms/Units</label>
            <input type="number" id="num_rooms" min="1" max="50" value="1"
                   class="w-full px-3 py-2 border rounded-md"
                   onchange="generateRoomInputs()">
            <p class="text-xs text-gray-500 mt-1">Maximum total quantity: <span id="max-qty-display">1</span></p>
          </div>
          ...
          <div class="mb-4 flex gap-2 flex-wrap">
            <button type="button" onclick="distributeEvenly()"
              ...
              <i class="fas fa-equals mr-1"></i>Distribute Evenly
            </button>
            <button type="button" onclick="setOnePerRoom()"
                    class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-semibold">
              <i class="fas fa-sort-numeric-up mr-1"></i>1 Per Room
            </button>
            <button type="button" onclick="setTwoPerRoom()"
                    class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-semibold">
              <i class="fas fa-sort-numeric-up mr-1"></i>2 Per Room
            </button>
          </div>

          <div id="room-inputs-container" class="space-y-3 max-h-96 overflow-y-auto"></div>

          <div class="mt-4 p-3 bg-indigo-600 rounded-lg">
            <div class="flex justify-between items-center gap-3">
              <span class="font-semibold text-white">Total Quantity:</span>
              <input type="number" id="distribution-total-input" min="1"
                     class="w-24 text-center text-lg font-bold text-indigo-700 border-2 border-indigo-300 rounded-lg px-2 py-1"
                     value="1" onchange="updateTotalQuantityFromModal(this.value)">
            </div>
            <p class="text-xs text-indigo-200 mt-1">
              <i class="fas fa-info-circle mr-1"></i>Editing this updates the main quantity field
            </p>
            <div class="mt-2">
              <span class="text-white text-sm">Distributed: </span>
              <span id="distribution-total" class="text-lg font-bold text-yellow-300">0</span>
              <span class="text-white text-sm"> / </span>
              <span id="distribution-max" class="text-lg font-bold text-white">1</span>
            </div>
            <div id="distribution-error" class="text-sm text-red-200 mt-1 hidden">
              <i class="fas fa-exclamation-triangle mr-1"></i>
              <span id="distribution-error-text"></span>
            </div>
          </div>

          <div class="mt-6 flex gap-3">
            <button type="button" onclick="applyRoomDistribution()"
                    class="flex-1 px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-semibold">
              <i class="fas fa-check mr-2"></i>Apply Distribution
            </button>
            <button type="button" onclick="closeRoomDistributionModal()"
...
// ─── Room Distribution ────────────────────────────────────────
let roomDistributionData = <?= json_encode(array_map(function($rd) {
    return [
        'room_number' => (int)$rd['room_unit_number'],
        'room_name'   => $rd['room_unit_name'] ?? '',
        'quantity'    => (int)$rd['quantity'],
        'notes'       => $rd['notes'] ?? '',
    ];
}, $existingRoomDist)) ?>;

function openRoomDistributionModal() {
  const qtyInput = document.querySelector('input[name="quantity"]');
  const totalQty = parseInt(qtyInput ? qtyInput.value : 1) || 1;
  document.getElementById('max-qty-display').textContent = totalQty;
  document.getElementById('distribution-total-input').value = totalQty;
  document.getElementById('distribution-max').textContent = totalQty;
  document.getElementById('distribution-total').textContent =
    roomDistributionData.reduce((sum, r) => sum + (r.quantity || 0), 0);

  if (roomDistributionData.length === 0) {
    roomDistributionData = [{ room_number: 1, room_name: '', quantity: totalQty, notes: '' }];
  }
  document.getElementById('num_rooms').value = roomDistributionData.length;
  generateRoomInputs();
  document.getElementById('roomDistributionModal').style.display = 'flex';
}

function closeRoomDistributionModal() {
  document.getElementById('roomDistributionModal').style.display = 'none';
}
...
function generateRoomInputs() {
  ...
  for (let i = 0; i < numRooms; i++) {
    newData.push(roomDistributionData[i] || { room_number: i + 1, room_name: '', quantity: 0, notes: '' });
  }
  roomDistributionData = newData;

  let html = '';
  roomDistributionData.forEach((room, index) => {
    html += `
      <div class="border border-gray-300 rounded-lg p-4 bg-white">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">Unit Number *</label>
            <input type="number" class="room-number-input w-full px-3 py-2 border rounded-md text-sm font-semibold"
                   data-index="${index}" value="${room.room_number}" min="1"
                   onchange="updateRoomData(${index}, 'room_number', parseInt(this.value) || 1)"
                   style="border-color:#6366f1; background:#f0f9ff;">
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">Unit Name (Optional)</label>
            <input type="text" class="room-name-input w-full px-3 py-2 border rounded-md text-sm"
                   data-index="${index}" value="${room.room_name || ''}"
                   placeholder="e.g. Master Bedroom"
                   onchange="updateRoomData(${index}, 'room_name', this.value)">
          </div>
        </div>
        <div class="mt-3">
          <label class="block text-xs font-medium text-gray-700 mb-1">Quantity *</label>
          <input type="number" class="room-qty-input w-full px-3 py-2 border rounded-md text-sm"
                 data-index="${index}" value="${room.quantity}" min="0"
                 onchange="updateRoomData(${index}, 'quantity', parseInt(this.value) || 0); validateDistribution();">
        </div>
        <div class="mt-2">
          <label class="block text-xs font-medium text-gray-700 mb-1">Notes (Optional)</label>
          <textarea class="room-notes-input w-full px-3 py-2 border rounded-md text-sm" rows="2"
                    data-index="${index}" placeholder="Additional notes"
                    onchange="updateRoomData(${index}, 'notes', this.value)">${room.notes || ''}</textarea>
        </div>
      </div>
    `;
  });
  document.getElementById('room-inputs-container').innerHTML = html;
  validateDistribution();
}

function updateRoomData(index, field, value) {
  if (roomDistributionData[index]) {
    roomDistributionData[index][field] = value;
    if (field === 'room_number' && parseInt(value) < 1) {
      roomDistributionData[index][field] = 1;
      document.querySelector(`input.room-number-input[data-index="${index}"]`).value = 1;
    }
  }
}
...
function validateDistribution() {
  const qtyInput = document.querySelector('input[name="quantity"]');
  const totalQty = parseInt(qtyInput ? qtyInput.value : 1) || 1;
  const distributedTotal = roomDistributionData.reduce((sum, r) => sum + (r.quantity || 0), 0);
  const totalDisplay = document.getElementById('distribution-total');
  const errorDiv = document.getElementById('distribution-error');
  const errorText = document.getElementById('distribution-error-text');

  if (totalDisplay) totalDisplay.textContent = distributedTotal;

  const unitNumbers = roomDistributionData.map(r => r.room_number);
  const duplicates = unitNumbers.filter((num, idx) => unitNumbers.indexOf(num) !== idx);
  if (duplicates.length > 0) {
    errorDiv.classList.remove('hidden');
    errorText.textContent = `Duplicate unit number: ${duplicates[0]}. Each unit must be unique.`;
    return false;
  }
  if (distributedTotal > totalQty) {
    errorDiv.classList.remove('hidden');
    errorText.textContent = `Distributed (${distributedTotal}) exceeds total (${totalQty})`;
    return false;
  }
...
function distributeEvenly() {
  const qtyInput = document.querySelector('input[name="quantity"]');
  const totalQty = parseInt(qtyInput ? qtyInput.value : 1) || 1;
  const numRooms = roomDistributionData.length;
  const baseQty = Math.floor(totalQty / numRooms);
  const remainder = totalQty % numRooms;
  roomDistributionData.forEach((room, i) => { room.quantity = baseQty + (i < remainder ? 1 : 0); });
  generateRoomInputs();
}

function setOnePerRoom() {
  const qtyInput = document.querySelector('input[name="quantity"]');
  const totalQty = parseInt(qtyInput ? qtyInput.value : 1) || 1;
  if (roomDistributionData.length > totalQty) { alert(`Cannot set 1 per room. Not enough quantity.`); return; }
  roomDistributionData.forEach((room, i) => { room.quantity = i < totalQty ? 1 : 0; });
  generateRoomInputs();
}

function setTwoPerRoom() {
  const qtyInput = document.querySelector('input[name="quantity"]');
  const totalQty = parseInt(qtyInput ? qtyInput.value : 1) || 1;
  if (roomDistributionData.length * 2 > totalQty) { alert(`Cannot set 2 per room. Not enough quantity.`); return; }
  roomDistributionData.forEach(room => { room.quantity = 2; });
  generateRoomInputs();
}

function updateTotalQuantityFromModal(newVal) {
  const qty = Math.max(1, parseInt(newVal) || 1);
  document.getElementById('distribution-total-input').value = qty;
  document.getElementById('distribution-max').textContent = qty;
  document.getElementById('max-qty-display').textContent = qty;
  const mainQty = document.querySelector('input[name="quantity"]');
  if (mainQty) mainQty.value = qty;
  validateDistribution();
}

function applyRoomDistribution() {
  if (!validateDistribution()) {
    const qtyInput = document.querySelector('input[name="quantity"]');
    const totalQty = parseInt(qtyInput ? qtyInput.value : 1) || 1;
    const distributedTotal = roomDistributionData.reduce((sum, r) => sum + (r.quantity || 0), 0);
    if (distributedTotal > totalQty) { alert('Distributed quantity exceeds total.'); return; }
    const unitNumbers = roomDistributionData.map(r => r.room_number);
    const duplicates = unitNumbers.filter((num, idx) => unitNumbers.indexOf(num) !== idx);
    if (duplicates.length > 0) { alert('Duplicate unit numbers found.'); return; }
  }

  const activeRooms = roomDistributionData.filter(r => r.quantity > 0);
  if (activeRooms.length === 0) { alert('Please assign quantities to at least one room/unit.'); return; }

  const form = document.querySelector('form');
  form.querySelectorAll('[name^="room_distribution"]').forEach(el => el.remove());

  activeRooms.forEach((room, index) => {
    ['room_number', 'room_name', 'quantity', 'notes'].forEach(field => {
      const input = document.createElement('input');
      input.type = 'hidden';
      input.name = `room_distribution[${index}][${field}]`;
      input.value = room[field] || '';
      form.appendChild(input);
    });
  });

  updateDistributionSummary(activeRooms);
  closeRoomDistributionModal();
}

function updateDistributionSummary(rooms) {
  const summary = document.getElementById('distribution-summary');
  const summaryList = document.getElementById('distribution-summary-list');
  if (!summary || !summaryList) return;

  let html = '<div class="space-y-1">';
  rooms.forEach(room => {
    const label = room.room_name || `Unit ${room.room_number}`;
    html += `<div class="flex justify-between text-xs">
      <span><i class="fas fa-door-open mr-1 text-blue-500"></i>${label} (Unit #${room.room_number})</span>
      <span class="font-semibold">${room.quantity} items</span>
    </div>`;
  });
  html += '</div>';
...
function clearRoomDistribution() {
  if (!confirm('Clear room/unit distribution?')) return;
