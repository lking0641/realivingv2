...
// Handle form submission for adding client from contact inquiry
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['contact_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        $update_stmt = $conn->prepare("UPDATE contact SET status = 'completed', client_id = ? WHERE id = ?");
        $update_stmt->bind_param("ii", $client_id, $contact_id);
        $update_stmt->execute();
        $update_stmt->close();
        $_SESSION['success_message'] = "Contact inquiry successfully converted to client!";
        header("Location: " . BASE_URL . "contact-clients");
        exit();
    } else {
...
// Get responded contact inquiries not yet completed
$pending_query = ($admin_role === 'superadmin')
    ? "SELECT c.*, acc.full_name as sales_name,
              (SELECT COUNT(*) FROM contact_inquiry_items WHERE contact_id = c.id) as items_count
       FROM contact c LEFT JOIN account acc ON c.assigned_to = acc.id
       WHERE c.status = 'responded' ORDER BY c.created_at DESC"
    : "SELECT c.*, acc.full_name as sales_name,
              (SELECT COUNT(*) FROM contact_inquiry_items WHERE contact_id = c.id) as items_count
       FROM contact c LEFT JOIN account acc ON c.assigned_to = acc.id
       WHERE c.status = 'responded' AND c.assigned_to = $admin_id ORDER BY c.created_at DESC";
...
$completed_query = ($admin_role === 'superadmin')
    ? "SELECT c.*, acc.full_name as sales_name,
              ...
              (SELECT COUNT(*) FROM contact_inquiry_items WHERE contact_id = c.id) as items_count
       FROM contact c LEFT JOIN account acc ON c.assigned_to = acc.id
       LEFT JOIN user_info ui ON c.client_id = ui.id
       WHERE c.status = 'completed' ORDER BY c.updated_at DESC"
    : "SELECT c.*, acc.full_name as sales_name,
              ui.reference_number, ui.clientname as client_name, ui.id as userinfo_id,
              (SELECT COUNT(*) FROM contact_inquiry_items WHERE contact_id = c.id) as items_count
       FROM contact c LEFT JOIN account acc ON c.assigned_to = acc.id
       LEFT JOIN user_info ui ON c.client_id = ui.id
       WHERE c.status = 'completed' AND c.assigned_to = $admin_id ORDER BY c.updated_at DESC";
...
.badge-product{background:var(--teal-bg);color:var(--teal);}
...
<div class="app-wrap">
  ...
  <div class="tab-content active" id="content-pending">
    <div class="section-card">
      ...
      <div style="overflow-x:auto;">
        <table class="apt-table">
          ...
          <tr>
            ...
            <td>
              <div style="font-size:13.5px;font-weight:500;"><?php echo htmlspecialchars($inq['subject']); ?></div>
              <?php if($inq['inquiry_type'] === 'contact_with_items'): ?>
              <div style="margin-top:4px;"><span class="badge badge-product"><i class="fas fa-box"></i> <?php echo $inq['items_count']; ?> Product(s)</span></div>
              <?php else: ?><div class="td-sub">General Contact</div><?php endif; ?>
              <div class="td-sub"><?php echo htmlspecialchars(substr($inq['message'],0,50)).(strlen($inq['message'])>50?'…':''); ?></div>
            </td>
      ...
      <?php else: ?>
      <div class="empty-state"><i class="fas fa-user-clock"></i><p>No responded inquiries ready for conversion</p><p style="font-size:13px;margin-top:6px;">Contacts must be in <strong>responded</strong> status before conversion</p></div>
      <?php endif; ?>
    </div>
  </div>
      ...
      <div style="overflow-x:auto;">
        <table class="apt-table">
          ...
          <tr>
            ...
            <td>
              <div style="font-size:13.5px;"><?php echo htmlspecialchars($client['subject']); ?></div>
              <?php if($client['inquiry_type'] === 'contact_with_items'): ?>
              <div style="margin-top:4px;"><span class="badge badge-product"><i class="fas fa-box"></i> <?php echo $client['items_count']; ?> Product(s)</span></div>
              <?php else: ?><div class="td-sub">General Contact</div><?php endif; ?>
            </td>
            <td>
...
<div class="modal-bg" id="convertModal">
  <div class="modal-box">
    <div class="modal-head">
      <h3><i class="fas fa-user-plus" style="color:var(--brand-light);"></i> Convert Contact to Client</h3>
    ...
    <form method="POST">
      ...
      <div class="form-group" style="margin-bottom:14px;"><label>Project Scope <span class="req">*</span></label><input type="text" name="project_scope" class="form-control" required placeholder="e.g., Commercial Interior Design"></div>
...
function openConvert(inq) {
  document.getElementById('modal_contact_id').value = inq.id;
  ...
  const inquiryType = inq.inquiry_type === 'contact_with_items'
    ? `Product Inquiry (${inq.items_count} item(s))`
    : 'General Contact';
  document.getElementById('modal_contact_details').innerHTML =
    `<strong>Subject:</strong> ${inq.subject} &nbsp;·&nbsp; <strong>Type:</strong> ${inquiryType}<br><strong>Message:</strong> ${inq.message}`;
