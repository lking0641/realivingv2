...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_image') {
        ...
        if ($del->execute()) {
            if ($img && file_exists(ROOT_PATH . "realiving_user/" . $img['image_path'])) {
                unlink(ROOT_PATH . "realiving_user/" . $img['image_path']);
            }
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <!-- ── Breadcrumb / Back ── -->
    <a href="manage-collections" class="inline-flex items-center gap-1.5 text-sm text-purple-600 hover:text-purple-800 mb-5 transition-colors">
    ...
    $img_disk_path = ROOT_PATH . 'realiving_user/' . ltrim($img['image_path'], './');
    $img_url_path  = CLIENT_ASSET . '/' . ltrim($img['image_path'], './');
?>
    <div class="img-card group">
        <?php if (file_exists($img_disk_path)): ?>
            <img src="<?php echo htmlspecialchars($img_url_path); ?>" alt="Gallery image" loading="lazy" />
                                <?php else: ?>
