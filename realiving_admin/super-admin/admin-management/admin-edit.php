...
if (!$edit_id) {
  $_SESSION['noti'] = "No admin selected.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Prevent locking yourself out: block removing your own superadmin role
  if (empty($errors) && $edit_id == $current_admin_id && $role !== 'superadmin') {
    $errors[] = "You cannot change your own role away from superadmin.";
  }
  ...
  if (empty($errors)) {
    ...
    if ($stmt->execute()) {
      $_SESSION['noti'] = "Admin account updated successfully.";
      header("Location: " . BASE_URL . "admin-management");
      exit;
    } else {
...
if (!$admin) {
  $_SESSION['noti'] = "Admin not found.";
  header("Location: " . BASE_URL . "admin-management");
  exit;
}

// If form was resubmitted with errors, keep the submitted values instead of DB values
...
<head>
  ...
  <title>Edit Admin - RealLiving</title>
  ...
  <div class="pt-10 pb-16 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto w-full">
    ...
    <div class="mb-8 adm-fade flex items-start justify-between flex-wrap gap-4">
      <div>
        ...
        <h1 class="adm-title">Edit Admin</h1>
        <p class="adm-subtitle mt-1">Update account details, role, and access.</p>
      </div>
      <a href="<?= BASE_URL ?>admin-management" class="adm-btn"><i class="fas fa-arrow-left"></i> Back to List</a>
    </div>
    ...
    <form method="POST" class="adm-card adm-fade">
      ...
      <div class="adm-field">
        ...
        <p class="adm-field-hint">Optional. Shown in place of full name in some views.</p>
      </div>
      ...
      <div class="adm-field">
        ...
        <?php if ($admin['id'] == $current_admin_id): ?>
          <p class="adm-field-hint">This is your own account — role must stay superadmin to avoid locking yourself out.</p>
        <?php endif; ?>
      </div>
      ...
      <div class="flex gap-2 mt-6">
        <a href="<?= BASE_URL ?>admin-management" class="adm-btn flex-1" style="justify-content:center;">Cancel</a>
