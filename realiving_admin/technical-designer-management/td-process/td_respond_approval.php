...
// Verify this approver is valid (GM, OM, designer head, or TD head)
...
// Update the td_attachment_approvals record
...
if ($updStmt->execute()) {
    ...
    // Check total required approvers
    ...
    // Count how many have approved this area/unit
    if ($room_unit_number !== null) {
    ...
    // If all approved → mark any active td_revision_log entry as approved
    if ($doneCount >= $totalRequired) {
        if ($room_unit_number !== null) {
            $markRevStmt = $conn->prepare("
                UPDATE td_revision_log SET status = 'approved'
